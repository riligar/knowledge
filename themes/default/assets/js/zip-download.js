...
class ZipDownloadManager {
  ...
  discoverMarkdownFiles() {
    ...
    const hasMainIndex = this.markdownFiles.some(
      (file) => file.path === "index.md" || file.path === "./index.md"
    );
    if (!hasMainIndex) {
  ...
  async createZipFile() {
    ...
    console.log(
      `Successfully added ${successCount}/${totalCount} markdown files to ZIP`
    );
  ...
  generateReadmeContent() {
    ...
    return `# Documentation - Documentação Completa

Este arquivo ZIP contém toda a documentação do Documentation em formato Markdown.
...
## Sobre o Documentation

Documentation é uma plataforma moderna e open-source para documentação que transforma como equipes criam, organizam e compartilham conhecimento.

Para mais informações, visite: https://github.com/riligar/documentation
`;
  }
  ...
  setLoadingState(loading) {
    ...
    if (downloadBtn) {
      if (loading) {
        ...
        downloadBtn.setAttribute(
          "title",
          "Download ZIP com toda a documentação"
        );
      }
    }
  }
